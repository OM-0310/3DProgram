...
void Character::ActionIdle::Update(Character& owner)
{
	...
	//owner.m_moveDir = Math::Vector3::Zero;
...
void Character::ActionWalk::Update(Character& owner)
{
	...
	//if (GetAsyncKeyState('D') & 0x8000)
	//{
	//	owner.m_moveDir.x = 1.0f;
	//}
	//else
	//{
	//	owner.m_IsMove = false;
	//}
	//if (GetAsyncKeyState('A') & 0x8000)
	//{
	//	owner.m_moveDir.x = -1.0f;
	...
	//if (_spCamera)
	//{
	//	owner.m_moveDir = owner.m_moveDir.TransformNormal(owner.m_moveDir, _spCamera->GetRotationYMatrix());
	//}
	//owner.m_moveDir.Normalize();
	//owner.m_moveDir *= owner.m_moveSpeed;
	//_nowPos += owner.m_moveDir;

	//// キャラクターの回転行列を創る
	//owner.UpdateRotate(owner.m_moveDir);
