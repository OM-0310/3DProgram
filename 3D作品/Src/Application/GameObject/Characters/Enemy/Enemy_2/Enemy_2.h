...
class Enemy_Main_2;
class Enemy_Gun_2;
class Enemy_Gun_NoMagazine_2;
class Enemy_Magazine_2;
class Player;
class AstarNode;
class AstarGrid;

class Enemy_2 : public CharaBase
{
public:
	...
	void SetEnemy_Main			
	(const std::shared_ptr<Enemy_Main_2>& _spEnemy_Main) { m_wpEnemy_Main = _spEnemy_Main; }

	void SetEnemy_Gun			
	(const std::shared_ptr<Enemy_Gun_2>& _spEnemy_Gun) { m_wpEnemy_Gun = _spEnemy_Gun; }

	void SetEnemy_Gun_NoMagazine
	(const std::shared_ptr<Enemy_Gun_NoMagazine_2>& _spEnemy_Gun_NoMgazine) { m_wpEnemy_Gun_NoMagazine = _spEnemy_Gun_NoMgazine; }

	void SetEnemy_Magazine			
	(const std::shared_ptr<Enemy_Magazine_2>& _spEnemy_Magazine) { m_wpEnemy_Magazine = _spEnemy_Magazine; }

	void SetPlayer				
	(const std::shared_ptr<Player>& _spPlayer) { m_wpPlayer = _spPlayer; }

	const bool& GetDissolveFlg		()	const	{ return m_dissolveFlg;		}
	const bool& GetAnswerFlg		()	const	{ return m_answerFlg;		}
...
private:
	...
	std::weak_ptr<Enemy_Main_2>				m_wpEnemy_Main;					// 敵(本体)情報
	std::weak_ptr<Enemy_Gun_2>				m_wpEnemy_Gun;					// 敵(銃)情報
	std::weak_ptr<Enemy_Gun_NoMagazine_2>	m_wpEnemy_Gun_NoMagazine;		// 敵(銃マガジン無し)情報
	std::weak_ptr<Enemy_Magazine_2>			m_wpEnemy_Magazine;				// 敵(マガジン)情報
	...
	const float		m_exeAnimFrm	= 40.0f;					// 処刑アニメーションフレーム 40.0f
	...
	bool			m_answerFlg		= false;					// 解答フラグ

	unsigned short	m_actionCnt		= 0;						// アクションカウント 0以上の整数のみを使用するためunsigned型で使用

	const short		m_maxShotWait	= 4;						// 発射待機時間MAX値
	short			m_shotWait		= 0;						// 発射待機時間

	bool			m_lostFlg		= false;					// 失踪フラグ
	short			m_lostCnt		= 0;						// 失踪カウント 0以上の整数のみを使用するためunsigned型で使用
	const short		m_lostCntMax	= 120;						// 失踪カウント最大値
	const short		m_lostCntMin	= 0;						// 失踪カウント最小値
...
private:
	...
	public:
		...
		virtual void Enter	(Enemy_2& _owner) { (void)_owner; }
		virtual void Update	(Enemy_2& _owner) { (void)_owner; }
		virtual void Exit	(Enemy_2& _owner) { (void)_owner; }
	};
	...
	public:
		...
		void Enter	(Enemy_2& _owner)	override;
		void Update	(Enemy_2& _owner)	override;
	};
	...
	public:
		...
		void Enter	(Enemy_2& _owner)	override;
		void Update	(Enemy_2& _owner)	override;
		void Exit	(Enemy_2& _owner)	override;
	};
