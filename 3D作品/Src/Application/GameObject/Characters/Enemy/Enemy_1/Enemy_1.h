...
class Enemy_Main_1;
class Enemy_Gun_1;
class Enemy_Gun_NoMagazine_1;
class Enemy_Magazine_1;
class Player;
class AstarNode;
class AstarGrid;

class Enemy_1 : public CharaBase
{
public:
	...
	void SetEnemy_Main			
	(const std::shared_ptr<Enemy_Main_1>& _spEnemy_Main) { m_wpEnemy_Main = _spEnemy_Main; }

	void SetEnemy_Gun			
	(const std::shared_ptr<Enemy_Gun_1>& _spEnemy_Gun) { m_wpEnemy_Gun = _spEnemy_Gun; }

	void SetEnemy_Gun_NoMagazine
	(const std::shared_ptr<Enemy_Gun_NoMagazine_1>& _spEnemy_Gun_NoMgazine) { m_wpEnemy_Gun_NoMagazine = _spEnemy_Gun_NoMgazine; }

	void SetEnemy_Magazine			
	(const std::shared_ptr<Enemy_Magazine_1>& _spEnemy_Magazine) { m_wpEnemy_Magazine = _spEnemy_Magazine; }
...
private:
	...
	std::weak_ptr<Enemy_Main_1>				m_wpEnemy_Main;					// 敵(本体)情報
	std::weak_ptr<Enemy_Gun_1>				m_wpEnemy_Gun;					// 敵(銃)情報
	std::weak_ptr<Enemy_Gun_NoMagazine_1>	m_wpEnemy_Gun_NoMagazine;		// 敵(銃マガジン無し)情報
	std::weak_ptr<Enemy_Magazine_1>			m_wpEnemy_Magazine;				// 敵(マガジン)情報
...
private:
	...
	public:
		...
		virtual void Enter	(Enemy_1& _owner) { (void)_owner; }
		virtual void Update	(Enemy_1& _owner) { (void)_owner; }
		virtual void Exit	(Enemy_1& _owner) { (void)_owner; }
	};
